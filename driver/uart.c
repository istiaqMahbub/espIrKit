...
uart_recvTask(os_event_t *events) {
	if (events->sig == 0) {
		...
		for(idx=0;idx<fifo_len;idx++) {
			...
			//os_printf("HW: [%d] %c", d_tmp, d_tmp);
			//gsm_hal_process_by_one_byte(d_tmp);
		}
...
STATUS ICACHE_FLASH_ATTR
uart0_tx_one_char_no_wait(uint8 TxChar) {
...
//copy uart buffer
LOCAL void ICACHE_FLASH_ATTR
Uart_Buf_Cpy(struct UartBuffer* pCur, char* pdata, uint16 data_len) {
	if (data_len == 0)
		return;
...
rx_buff_deq(char* pdata, uint16 data_len) {
	...
	uint16 tail_len = pRxBuffer->pUartBuff + pRxBuffer->UartBuffSize
			- pRxBuffer->pOutPos;
	uint16 len_tmp = 0;
...
uart0_Available() {
	return pRxBuffer->pInPos != pRxBuffer->pOutPos;
}
	...
	if (buf_len > 0) {
		...
		if (pRxBuffer->pOutPos <= pRxBuffer->pInPos) {
			...
			if (len_tmp > tail_len) {
				...
				os_memcpy(pdata + tail_len, pRxBuffer->pOutPos,
						len_tmp - tail_len);
				pRxBuffer->pOutPos += (len_tmp - tail_len);
				pRxBuffer->pOutPos = (pRxBuffer->pUartBuff
						+ (pRxBuffer->pOutPos - pRxBuffer->pUartBuff)
...
//--------------------------------
LOCAL void ICACHE_FLASH_ATTR
tx_fifo_insert(struct UartBuffer* pTxBuff, uint8 data_len,
		uint8 uart_no) {
	uint8 i;

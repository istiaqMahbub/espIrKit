...
mesh_device_disp_mac_list()
{
    ...
    for (idx = 0; idx < g_node_list.scale - 1; idx ++)
    {
		...
		add_unique_meter(g_node_list.list[idx].mac);
        }*/
    }
    MESH_DEMO_PRINT("=====mac list end======\n");
    /*if (!espconn_mesh_is_root()) {
		uint8_t listData[ESP_MESH_ADDR_LEN*10];//TODO: need better data structure
		uint8_t list_len = 0;
		if(find_delete_create_inactive_meter_list(10000, listData, &list_len))
		{
			os_printf("\r\nInactive Meter list created, Len: %d\r\n", list_len);
		}
    }*/
}

...
void Softuart_EnableRs485(Softuart *s, uint8_t gpio_id) {
	...
	//enable pin as gpio
	PIN_FUNC_SELECT(softuart_reg[gpio_id].gpio_mux_name,
			softuart_reg[gpio_id].gpio_func);
...
// disable interrupt for GPIO0
		gpio_pin_intr_state_set(GPIO_ID_PIN(s->pin_rx.gpio_id),
...
// Do something, for example, increment whatyouwant indirectly
		...
		if (!level) {
			...
			//TODO: following block should be done upon parity enabled settings
			//checking for parity bit
			while ((0x7FFFFFFF & system_get_time())
					< (start_time + (s->bit_time * (i + 1)))) {
				//If system timer overflow, escape from while loop
				if ((0x7FFFFFFF & system_get_time()) < start_time) {
			...
			uint8 next = (s->buffer.receive_buffer_tail + 1)
...
uint8_t Softuart_Read(Softuart *s) {
	...
	s->buffer.receive_buffer_head = (s->buffer.receive_buffer_head + 1)
			% SOFTUART_MAX_RX_BUFF;
	return d;
}
...
BOOL Softuart_Available(Softuart *s) {
	return (s->buffer.receive_buffer_tail + SOFTUART_MAX_RX_BUFF
			- s->buffer.receive_buffer_head) % SOFTUART_MAX_RX_BUFF;
}
...
static inline u8 calculateparity(char scancode) {
	u8 parity = 0;
...
void Softuart_txEnable(Softuart *s, u8 enable) {
	if (s->is_rs485 == 1) {
		GPIO_OUTPUT_SET(GPIO_ID_PIN(s->pin_rs485_tx_enable), enable);
	}
}

// Function for printing individual characters
void Softuart_Putchar(Softuart *s, char data, bool is_last) {
	unsigned i;
	unsigned start_time = 0x7FFFFFFF & system_get_time();
	u8 parity = calculateparity(data);
	//os_printf("\r\nbyte = %d, parity = %d",data,parity);
	...
	if (parity) { //if true then parity bit should be 1 for even parity
		GPIO_OUTPUT_SET(GPIO_ID_PIN(s->pin_tx.gpio_id), 1);
		//os_printf("EVEN\r\n");
	} else {
		GPIO_OUTPUT_SET(GPIO_ID_PIN(s->pin_tx.gpio_id), 0);
